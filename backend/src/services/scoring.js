...
function casEngagement(followers, avgLikes, avgComments, avgShares=0, avgSaves=0, platform='instagram') {
  ...
  // YouTube has naturally lower ER due to massive subscriber counts
  // 0.3% ER on 100M+ subs is excellent; scale accordingly
  let threshold;
  if (platform === 'youtube') {
    threshold = followers > 10_000_000 ? 0.5   // mega: 0.5% = 100
              : followers > 1_000_000  ? 1.0   // macro: 1% = 100
              : followers > 100_000    ? 2.0   // mid: 2% = 100
              :                          4.0;  // micro: 4% = 100
  } else {
    threshold = 8; // Instagram standard
  }
